...
class SubtaskWithTimingColumnsManager(Manager):
    ...
    def with_maximum_download_time(self):  # pylint: disable=no-self-use
        bytes_per_sec = settings.MINIMUM_UPLOAD_RATE << 10
        download_time = Func(F('result_package_size') / float(bytes_per_sec), function='CEIL')
        return Value(settings.DOWNLOAD_LEADIN_TIME) + download_time

    def with_subtask_verification_time(self):  # pylint: disable=no-self-use
        ...
        subtask_verification_time = (4 * settings.CONCENT_MESSAGING_TIME) + (3 * F('maximum_download_time')) + (
                0.5 * subtask_timeout)
        return ExpressionWrapper(subtask_verification_time, output_field=IntegerField())

    def with_download_deadline(self):  # pylint: disable=no-self-use
        computation_deadline = F('computation_deadline')
        download_deadline = Func(Value('epoch'), computation_deadline, function='DATE_PART') + F(
            'subtask_verification_time')
        return ExpressionWrapper(download_deadline, output_field=IntegerField())

    def with_timing_columns(self):
        return super().get_queryset().annotate(
            maximum_download_time=self.with_maximum_download_time(),
            subtask_verification_time=self.with_subtask_verification_time(),
            download_deadline=self.with_download_deadline(),
        )

    def get_queryset(self):
        return self.with_timing_columns()
